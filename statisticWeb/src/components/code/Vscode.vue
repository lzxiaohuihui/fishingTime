...
export default {
    ...
    methods: {
        async getData() {
            ...
            const res = await this.$http.get('http://localhost:8888/getVscodeRunningTime/' + this.startDay + '_' + this.endDay)
            var arrx = []
            var arry = []
            var sum = 0
            ...
            const option = {
                title: { text: this.startDay + '~' + this.endDay + "     VSCode使用情况     " + total },
                xAxis: {
                    type: 'value',
                    max: 'dataMax',
                    name: 'time',
            ...
            setInterval(() => {
                axios.get('http://localhost:8888/getVscodeRunningTime/' + this.startDay + '_' + this.endDay).then((r) => {
                    var x = []
                    var y = []
                    var sum = 0
